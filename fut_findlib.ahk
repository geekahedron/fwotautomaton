FindPatternInArea(area,err1,err0,pattern,debug:=true)
{
	global coords
	p := StrSplit(StrSplit(pattern, "<")[2],">")[1]
	a := coords[area]
	x1 := a[1][1]
	y1 := a[1][2]
	x2 := a[2][1]
	y2 := a[2][2]
	; MsgBox %area%: %x1%,%y1%-%x2%,%y2%`n%wX%,%wY%`n %pattern%

	; WinGetPos, wX, wY,,, BlueStacks
	WinGetPos, wX, wY,,, A
	result := {}
	if (arr := FindPatternR(x1,y1,x2-x1,y2-y1,err1,err0,pattern,debug))
	{
		result["x"] := arr.1.1 + (arr.1.3//2)
		result["y"] := arr.1.2 + (arr.1.4//2)
		result["c"] := arr.1.5
		
		c := 1
		for k,v in arr
		{
			result[c++] := [v.1,v.2,v.3,v.4,v.5]
;			if (debug)
			Debug("Result: " . v.1 . "," . v.2 . ":" . v.3 . "x" . v.4 . "::" . v.5)
		}
;		if (debug)
			Debug("R1: " . result["x"] . "," . result["y"] . ":" . result["c"])
	}
	else
	{
		if (debug)
			Screenshot(,x1,y1,x2,y2,,p . "-nf")
		result := 0
	}
	return result
}

; x, y are top left coordinates of search box
; w, y are dimensions of search box
; err1 is the character "0" fault-tolerant in percentage.
; err0 is the character "_" fault-tolerant in percentage.
; Pattern can be a lot of text to find, separated by "|".
; return is a array, contains the [X,Y,W,H,Comment] results of Each Find.

FindPatternR(x,y,w,h,err1,err0,pattern,debug:=true)
{
	if (debug)
		Debug("FindPatternR(" . x . "," . y . "|" . w . "x" . h . ";" . debug . ": " . pattern)
	p := StrSplit(StrSplit(pattern, "<")[2],">")[1]
	x1 := x
	y1 := y
	; WinGetPos, wX, wY,,, BlueStacks
	WinGetPos, wX, wY,,, A
	xywh2xywh(x+wX,y+wY,w,h,x,y,w,h)
	if (w<1 or h<1)
		return, 0
	bch:=A_BatchLines
	SetBatchLines, -1
	...
	Loop, Parse, pattern, |
	{
		...
		;--------------------------------------------
		if (ok:=PicFind(mode,color,n,Scan0,Stride,sx,sy,sw,sh,v,s1,s0,Round(len1*e1),Round(len0*e0),w1,h1,allpos)) or (err1=0 and err0=0 and (ok:=PicFind(mode,color,n,Scan0,Stride,sx,sy,sw,sh,v,s1,s0,Round(len1*0.05),Round(len0*0.05),w1,h1,allpos)))
		{
			Loop, % ok
				pos:=NumGet(allpos, 4*(A_Index-1), "uint"), rx:=(pos&0xFFFF)+x, ry:=(pos>>16)+y,arr.Push( [rx,ry,w1,h1,Comment] )
		}
	}
	SetBatchLines, %bch%
...
;	if !(arr.MaxIndex())
;		Screenshot(,x1,y1,x1+w,y1+h,,p . "-nf")
...
FindPattern(x,y,w,h,err1,err0,text)
{
	xywh2xywh(x-w,y-h,2*w+1,2*h+1,x,y,w,h)
;	Debug("FindPattern(" . x . "," . y . "|" . w . "x" . h . ": " . text)
	if (w<1 or h<1)
		return, 0
	bch:=A_BatchLines
	SetBatchLines, -1
...
{
  OCR:="", Right_X:=nX+nW
  While (ok:=FindPattern(nX, nY, nW, nH, err1, err0, Text))
  {
